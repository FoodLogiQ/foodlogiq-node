...
module.exports = function(accessToken, businessId) {
  ...
  this.listByType = function(callback, type){
    var conn = fc.init(accessToken);
    conn.get('/businesses/'+businessId+'/events/' + type, function(err, events) {
      if(err) return callback(err);
      callback(null, events);
    });
  }
  ...
  this.delete = function(eventId, callback){
    var conn = fc.init(accessToken);
    conn.delete('/businesses/'+businessId+'/events' + eventId, event, function(err, newEvent) {
      if(err) return callback(err);
      callback(null, newEvent);
    });
  };
